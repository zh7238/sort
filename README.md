...
本節以「固定測時方法 + 調整輸入規模 N」觀察五種排序法的時間成長趨勢。所有實驗建議使用相同電腦、相同編譯參數，並對每個 N 重複多次取平均，降低誤差。
...
| 插入排序（逆序） | 1000 |  |  | O(n^2) |
| 合併排序 | 2000 |  |  | O(n log n) |
| 快速排序（隨機） | 2000 |  |  | O(n log n) |

#### 結論撰寫建議
1. 先比較 O(n^2) 三種排序法在 N 倍增時的時間倍率。
2. 再比較 O(n log n) 的合併排序與快速排序在大 N 時的優勢。
3. 補充快速排序在最差情況會退化、插入排序在近乎有序資料會加速的現象。
