...
這次排序報告的實作與實驗讓我從程式層面真正理解排序法差異。透過不同資料規模的測試，我觀察到 O(n^2) 演算法在資料量增加時耗時成長明顯，而 O(n log n) 演算法在大規模資料下更有效率，與理論複雜度相同。除此之外，我也理解到實際效能不只取決於理論，還會受到資料分布、實作方式與執行環境影響。這次做的視覺化模擬頁面也讓我學到了如何使用AI協作來幫助自己更有效的理解想學習的知識，未來在學習其他課堂內容時，如果遇到不好理解的地方，不管是什麼科目我都會嘗試使用AI協作寫出模擬頁面等程式幫助自己了解課堂內容。
